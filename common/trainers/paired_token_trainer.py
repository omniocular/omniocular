        ...
        for batch_idx, batch in enumerate(self.train_loader):
            ...
            if hasattr(self.model, 'tar') and self.model.tar:
                if 'ignore_lengths' in self.config and self.config['ignore_lengths']:
                    scores, rnn_outs = self.model(batch.code1, batch.code2)
                else:
                    scores, rnn_outs = self.model(batch.code1[0], batch.code2[0], lengths=(batch.code1[1], batch.code2[1]))
            else:
                if 'ignore_lengths' in self.config and self.config['ignore_lengths']:
                    scores = self.model(batch.code1, batch.code2)
                else:
                    scores = self.model(batch.code1[0], batch.code2[0], lengths=(batch.code1[1], batch.code2[1]))
