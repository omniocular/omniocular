...
class KimCNN(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.fc1 = nn.Linear(filter_widths * output_channel, target_class)
    ...
    def forward(self, x, **kwargs):
        ...
        x = [F.relu(self.conv1(x)).squeeze(3),
             ...
             F.relu(self.conv3(x)).squeeze(3)]  # (batch, channel_output, ~=sent_len) * filter_widths
        x = [F.max_pool1d(i, i.size(2)).squeeze(2) for i in x]  # (batch, channel_output) * filter_widths
