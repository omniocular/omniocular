...
class ApacheDiffToken(TabularDataset):
    ...
    @classmethod
    def splits(cls, path, train=os.path.join('apache_diff_token', 'train_bal.tsv'),
               validation=os.path.join('apache_diff_token', 'dev_bal.tsv'),
               test=os.path.join('apache_diff_token', 'test_bal.tsv'), **kwargs):
        return super(ApacheDiffToken, cls).splits(
